...
export default class SettingsTabPanel extends React.Component {
	...
	renderBasedOnStartDateOption() {
		...
		return (
			<div
				className="preview-option preview-mode-none"
				onClick={this.nullOutPreviewMode}
			>
	...
	renderOptions() {
		return (
			<div className="course-options">
				<div className="publicly-available-option">
					{this.renderOption(
						t('publiclyAvailable'),
						null,
						!this.state.isNonPublic,
						this.onPublicChange
					)}
				</div>
				<div className="preview-mode-option">
					<div className="course-option">
						<div className="course-option-label">
							{t('previewMode')}
						</div>
						{this.renderPreviewWidet()}
					</div>
				</div>
			</div>
		);
