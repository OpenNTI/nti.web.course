...
export default class CourseEnrollmentAdmin extends React.Component {
	...
	render() {
		...
		const active =
			(stages && stages.find(stage => !stage.isDone(this.state))) || {};
		...
		return (
			<div className="course-enrollment-admin-view">
				...
				{active.name === 'manage' &&
					this.renderManageEnrollment(user, course)}
			</div>
		);
	}

	renderUser() {
		return <PickUser onUserSelected={this.onUserSelected} />;
	}

	renderCourse() {
		return <PickCourse onCourseSelected={this.onCourseSelected} />;
	}
