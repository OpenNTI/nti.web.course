...
const t = scoped('course.enrollment.admin.manage-enrollment.option', {
	...
	dropConfirmation: {
		title: 'Are you sure?',
		description: 'Removing the user will take away their access to the course material and remove all their course work.',
		cancel: 'Cancel',
		continue: 'Remove'
	},
	notAvailableWarning: {
...
const getDisclosureLocaleRoot = (d) => {
	if (d === DROP_CONFIRMATION) { return 'dropConfirmation'; }
	if (d === NOT_AVAILABLE_WARNING) { return 'notAvailableWarning'; }
};

const getDisclosureButtonCls = (d) => {
	if (d === DROP_CONFIRMATION) { return 'drop'; }
	if (d === NOT_AVAILABLE_WARNING) { return 'not-available'; }
};
...
export default class CourseEnrollmentAdminManageEnrollmentOption extends React.Component {
	...
	renderDescription (option) {
		const enrolled = option.isEnrolled();
		const description = enrolled ? option.getEnrolledDescription() : option.getDescription();

		return (
			<div className="description" {...rawContent(description)} />
		);
	}
	...
	renderButton (option) {
		...
		return (
			<Button className={cx({enrolled})} rounded onClick={this.toggleEnrollment}>
		...
		return (
			<div className={cx('disclosure', buttonCls)}>
				...
				<div className="message">
					...
					<div className="description">{t(`${localeRoot}.description`)}</div>
				</div>
				<div className="buttons">
					<Button className="cancel" rounded secondary onClick={this.cancelAction}>{t(`${localeRoot}.cancel`)}</Button>
					<Button className={buttonCls} rounded onClick={this.toggleEnrollment}>{t(`${localeRoot}.continue`)}</Button>
				</div>
			</div>
		);
	}
