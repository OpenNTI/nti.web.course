...
class CourseEnrollmentAdminManageEnrollment extends React.Component {
	...
	renderNotAuthorized() {
		return <EmptyState header={t('notAuthorized')} />;
	}
	...
	renderRecord(record) {
		...
		const listOptions = record
			? options.filter(o => o.isEnrolled())
			: options;
			...
			<div>
				<div className={cx('enrolled-header', { enrolled: !!record })}>
					...
					<span className="label">
						{record ? t('enrolled') : t('notEnrolled')}
					</span>
				</div>
