...
describe('Course enrollment open option test', () => {
	...
	test('Test getDescription', () => {
		...
		enrollment = new Option(
			{ getEnrollCutOffDate: () => new Date('10/22/2017') },
			null,
			basicCatalogEntry
		);
		expect(enrollment.getDescription()).toEqual(
			'Earn transcripted college credit from the University of Oklahoma.  Not available after October 22nd, 2017.'
		);
	});
	...
	test('Test getEnrolledDescription', () => {
		...
		expect(
			enrollment.getEnrolledDescription().indexOf('Class begins ')
		).toEqual(0);
		expect(
			enrollment
		...
		enrollment = new Option({ enrolled: false }, null, {
			getEndDate: () => new Date(Date.now() + 1000 * 60 * 60 * 48),
			getStartDate: () => null,
		});
		expect(enrollment.getEnrolledDescription()).toEqual(
	...
	test('Test getEnrollButtonLabel', () => {
		...
		expect(enrollment.getEnrollButtonLabel(true)).toEqual(
	...
	test('Test getPendingDescription', () => {
		let enrollment = new Option({}, null, basicCatalogEntry);
		expect(enrollment.getPendingDescription()).toEqual(
	...
	test('Test getRejectedTitle', () => {
		let enrollment = new Option({}, null, basicCatalogEntry);
		expect(enrollment.getRejectedTitle()).toEqual(
	...
	test('Test getRejectedDescription', () => {
		let enrollment = new Option({}, null, basicCatalogEntry);
		expect(enrollment.getRejectedDescription()).toEqual(
	...
	test('Test getApiDownDescription', () => {
		let enrollment = new Option({}, null, basicCatalogEntry);
		expect(enrollment.getApiDownDescription()).toEqual(
	...
	test('Test getAvailabelSeatsLabel', async () => {
		let enrollment = new Option(
			{
				available: true,
				fetchLink: () => ({ Course: { SeatAvailable: 5 } }),
			},
			null,
			basicCatalogEntry
		);
		await enrollment.load();
		expect(enrollment.getAvailabelSeatsLabel()).toEqual(
			'5 Seats Available'
		);
		...
		enrollment = new Option(
			{
				available: true,
				fetchLink: () => ({ Course: { SeatAvailable: 0 } }),
			},
			null,
			basicCatalogEntry
		);
		...
		enrollment = new Option(
			{
				available: true,
				fetchLink: () => ({ Course: { SeatAvailable: 1 } }),
			},
			null,
			basicCatalogEntry
		);
	...
	test('Test getDropInfoDescription', () => {
		let enrollment = new Option({}, null, basicCatalogEntry);
		expect(enrollment.getDropInfoDescription()).toEqual(
	...
	test('Test getGetAcquaintedWith', () => {
		let enrollment = new Option({}, null, basicCatalogEntry);
		expect(enrollment.getGetAcquaintedWith()).toEqual(
	...
	test('Test getCompleteProfile', () => {
		let enrollment = new Option({}, null, basicCatalogEntry);
		expect(enrollment.getCompleteProfile()).toEqual(
