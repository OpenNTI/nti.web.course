...
describe('Course enrollment open option test', () => {
	test('Test getTitle', () => {
		let enrollment = new Option({ enrolled: false }, null, { getEndDate: () => null, getStartDate: () => null});
		expect(enrollment.getTitle()).toEqual('Basic');

		enrollment = new Option({ enrolled: false }, null, { getEndDate: () => new Date('10/31/2017'), getStartDate: () => new Date('10/22/2017')});
		expect(enrollment.getTitle()).toEqual('Archived');
	});

	test('Test getEnrolledTitle', () => {
		let enrollment = new Option({ enrolled: true }, null, { getEndDate: () => null, getStartDate: () => null});
		expect(enrollment.getTitle()).toEqual('Basic');

		enrollment = new Option({ enrolled: true }, null, { getEndDate: () => new Date('10/31/2017'), getStartDate: () => new Date('10/22/2017')});
		expect(enrollment.getTitle()).toEqual('Archived');
	});

	test('Test getDescription', () => {
		let enrollment = new Option({ enrolled: false }, null, { getEndDate: () => null, getStartDate: () => null});
		expect(enrollment.getDescription()).toEqual('Interact with content and connect with a community of learners.');
	});
	...
	test('Test getEnrolledDescription', () => {
		...
		enrollment = new Option({ enrolled: false }, null, { getEndDate: () => null, getStartDate: () => null});
		...
		expect(enrollment.getEnrolledDescription().indexOf('Class begins October 22nd')).toEqual(0);
	...
	test('Test getEnrollButtonLabel', () => {
		let enrollment = new Option({}, null, { getEndDate: () => new Date('10/31/2017'), getStartDate: () => new Date('10/22/2017')});
