...
const ATTRIBUTES = [
	...
	(c, o) => o && o.getPrice ? 'hasPrice' : 'noPrice',
	(c, o) => o && o.getEnrollCutOffDate && o.getEnrollCutOffDate() ? 'hasCutOff' : 'noCutOff',
	(c, o, a) => {
...
export default function getTranslationFor (t, base, catalogEntry, option, access) {
	const attributes = ATTRIBUTES.map(getKey => getKey(catalogEntry, option, access)).filter(key => key);
	const keyCombos = combinations(attributes).sort((a, b) => b.length - a.length);
