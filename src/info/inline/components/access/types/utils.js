export const getPurchaseOption = course => course?.getEnrollmentOptions()?.getEnrollmentOptionForPurchase();
export const getOpenOption = course => course?.getEnrollmentOptions()?.getEnrollmentOptionForOpen();

export const getPurchasable = course => getPurchaseOption(course)?.getPurchasable();
export const getPriceFromPurchasable = purchasable => ({amount: (purchasable?.amount ?? 0) * 100, currency: purchasable.currency});
export const getPrice = course => {
	const option = getPurchaseOption(course);
	const purchasable = getPurchasable(course);

	return purchasable && option?.enabled ? getPriceFromPurchasable(purchasable) : null;
};
...
export const setPrice = async (course, price) => {
	...
	const saving = purchasable ?
		...
		course.postToLink('CreateCoursePurchasable', {Amount: dollars, Currency: currency});
