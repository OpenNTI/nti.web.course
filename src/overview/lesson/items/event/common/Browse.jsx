		...
		if(!events || events.length === 0) {
			return (
				<div className="empty-state">
					<div className="no-events">{searchTerm ? t('noEventsForFilter') : t('noEvents')}</div>
					{!searchTerm && <div className="add-new" onClick={() => this.props.onSelect()}>{t('addNew')}</div>}
				</div>
			);
		}
			...
			<div className="event-browser">
				<div className="toolbar">
					<div className="search">
						<Input.Text value={searchTerm} onChange={(val) => doSearch(val)} placeholder={t('searchPlaceholder')}/>
						<i className="icon-search"/>
					</div>
					<div className="create-new" onClick={() => this.props.onSelect()}><i className="icon-add"/></div>
				</div>
				{loading && <div className="loading"><Loading.Ellipsis/></div>}
