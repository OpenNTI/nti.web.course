...
export default function LessonOverviewAssignmentIcon({
	assignment,
	assignmentHistory,
	large,
}) {
	...
	const completed =
		assignmentHistory?.Submission?.getCreatedTime() ??
		CompletedItem?.getCompletedDate();
	...
	if (completed) {
		cls = completed > due ? 'late' : 'on-time';
	}
	if (due && due < now) {
		cls = 'late';
	}
	...
	return (
		<div
			className={cx('lesson-overview-assignment-icon', cls, {
				large,
				['no-completion']: completed && !CompletedItem,
			})}
		/>
	);
}
