...
export default class WebinarBaseItem extends React.Component {
	...
	renderDate() {
		...
		return (
			<DateTime.DateIcon minimal className="date" date={startDate}>
				{item.hasCompleted() && !noProgress && isMinimal && (
	...
	renderStatus(startTime, endTime) {
		...
		return (
			<div className="status">
				...
				<div
					className={cx('duration-container', {
						'is-active': isAvailable,
					})}
				>
					<DateTime.Duration {...{ startTime, endTime }} />
				</div>
			</div>
		);
	}
		...
		return (
			...
			>
				{hasIcon && (
					<div className="image">
						<img src={item.icon} />
						{this.renderStatus(startDate, endDate)}
					</div>
				)}
		...
		return (
			<div className="contents">
				<div className="header">
					<div className="title">
						{webinar ? webinar.subject : t('noLongerAvailable')}
					</div>
					<Event.Availability
						startTime={startDate}
						endTime={endDate}
				...
				{!readOnly && !hideControls && !noProgress && (
					<Button
						webinar={webinar}
				...
				{webinar &&
					!isMinimal &&
					this.renderImageAndDescription(startDate, endDate)}
			</div>
		);
	}
	...
	onStatusChange = status => {
		if (this.state.status !== status) {
			...
			this.props.item.webinar.refresh().catch(() => {
