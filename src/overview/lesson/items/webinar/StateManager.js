...
export default class WebinarStateManager extends EventEmitter {
	...
	schedule(timeout) {
		...
		this.timeout = setTimeout(() => {
			this.calculateState();
		}, Math.min(timeout, 24 * 60 * 60 * 1000));
	}

	isSessionStartingWithinMinute(session, now) {
		return (
			session.getStartTime() > now &&
			session.getStartTime() - MINUTE_THRESHOLD < now
		);
	}

	isSessionStartingInMoreThanAMinute(session, now) {
		return (
			!this.isSessionStartingWithinMinute(session, now) &&
			session.getStartTime() > now &&
			session.getStartTime() - COUNTDOWN_THRESHOLD < now
		);
	}
	...
	isSessionExpiringSoon(session, now) {
		return (
			session.getEndTime() >= now &&
			session.getEndTime() - COUNTDOWN_THRESHOLD <= now
		);
	}
	...
	async calculateState() {
		...
		const isUnregistered =
			this.webinar &&
			!this.webinar.isExpired() &&
			!this.webinar.isJoinable() &&
			this.webinar.hasLink('WebinarRegistrationFields');
		...
		const unJoinable =
			!this.webinar ||
			(!this.webinar.isExpired() &&
				!this.webinar.hasLink('JoinWebinar') &&
				!this.webinar.hasLink('WebinarRegistrationFields'));
		...
		if (this.webinar.isExpired() || unJoinable) {
			...
			newState = {
				currentState: isUnregistered
					? States.UnregisteredStartingInMinute
					: States.RegisteredStartingInMinute,
			};

			this.schedule(
				Math.min(MINUTE_THRESHOLD, nearestSession.getStartTime() - now)
			); // recalculate when we expect the webinar to start
		} else if (this.isSessionFarFromStarting(nearestSession, now)) {
			newState = {
				currentState: isUnregistered
					? States.Unregistered
					: States.RegisteredInactive,
			};

			this.schedule(
				nearestSession.getStartTime() - COUNTDOWN_THRESHOLD - now
			); // recalculate when we get to the countdown threshold
		} else if (
			this.isSessionStartingInMoreThanAMinute(nearestSession, now)
		) {
			newState = {
				currentState: isUnregistered
			...
			newState = {
				currentState: isUnregistered
					? States.Unregistered
					: States.RegisteredActive,
			};

			this.schedule(
				nearestSession.getEndTime() - COUNTDOWN_THRESHOLD - now
			); // recalculate when we hit the expiration countdown threshold
