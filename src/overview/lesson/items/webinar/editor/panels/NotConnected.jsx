...
const t = scoped('course.overview.lesson.items.webinar.editor.panels.NotConnected', {
	...
	description: 'Connecting a GoToWebinar account will allow authors to add webinars to any course.  Managing integrations can be found in the Site Administration view.',
	connect: 'Connect'
});
...
export default class WebinarNotConnected extends React.Component {
	...
	render () {
		if(this.state.loading) {
			return <div className="webinar-not-connected"><Loading.Ellipsis/></div>;
		}
		...
		return (
			<div className="webinar-not-connected">
				...
				{this.state.isSiteAdmin && <div onClick={this.onConnect} className="connect-button">{t('connect')}</div>}
			</div>
		);
	}
}
