...
const t = scoped(
	'course.overview.lesson.items.webinar.editor.panels.Registration',
	{
		title: 'Paste or Enter a Registration Link',
		description:
			'After you create a webinar in GoToWebinar you can add it to your course by providing a registration link.',
		...
		addAsResourceDesc:
			'Reporting, completion controls, and other benefits of our webinar integration will not be available.',
		addAsResourceLink: 'Yes, add anyway.',
		multipleInstances:
			'There is more than one occurrence of this webinar.  Please choose one.',
	}
);
...
export default class WebinarRegistrationEditor extends React.Component {
	...
	renderPasteForm() {
		...
		return (
			<div className="paste-form">
				...
				{!this.state.learnMoreExpanded && (
					<div onClick={this.showMoreInfo} className="learn-more">
						{t('learnMore')}
					</div>
				)}
				{this.state.learnMoreExpanded && this.renderSteps()}
				<GotoWebinar.Input.Text
	...
	renderWebinarOption = webinar => {
		...
		return (
			...
			>
				<BaseItem item={item} isMinimal hideControls />
			</div>
		);
	};
	...
	renderChooseWebinar() {
		return (
			<div className="choose-webinar">
				...
				<div className="webinar-options">
					{this.state.webinars.map(this.renderWebinarOption)}
				</div>
			</div>
		);
	}
	...
	render() {
		...
		const cls = cx('webinar-registration-editor', {
			error: Boolean(error),
		});
