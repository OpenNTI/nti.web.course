...
const isNoSubmitAssignment = ({ assignment: a, assignmentHistory: h }) =>
...
const isAssignmentCompleted = ({ assignment }) =>
	Boolean(assignment?.CompletedItem);

const getAssignmentGradedDate = ({ assignmentHistory: h }) =>
	h?.grade?.getCreatedTime();
...
const Statuses = [
	...
	{
		handles: props => {
			if (!isAssignment(props) || !isSubmittedAssignment(props)) {
				return false;
			}
	...
	//Assignment Graded
	{
		...
		getLabel: props =>
			t('graded', { date: formatDate(getAssignmentGradedDate(props)) }),
	},
	...
	//Assignment Overdue
	{
		handles: props => {
			if (
				!isAssignment(props) ||
				isNoSubmitAssignment(props) ||
				isSubmittedAssignment(props)
			) {
				return false;
			}
		...
		getLabel: props =>
			t('overdue', { date: formatDate(getAssignmentDueDate(props)) }),
	},
...
CompletionStatus.completedSuccessfully = item =>
	item?.completedSuccessfully?.() ?? item?.CompletedItem?.Success === true;
