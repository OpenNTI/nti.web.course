...
class OutlineHeader extends React.Component {
	...
	async loadAdminProgress(course) {
		try {
			...
			const subLabel =
				studentsFinished === 1
					? '1 Student Finished'
					: studentsFinished + ' Students Finished';
	...
	isStudent(course) {
		...
		return (
			PreferredAccess &&
			/courseinstanceenrollment/.test(PreferredAccess.MimeType)
		);
	}
	...
	async loadProgress(course, force) {
		if (!this.shouldLoad(course, force)) {
			return;
		}
					...
					<Prompt.Dialog onBeforeDismiss={this.hideCertificate}>
						<Iframe
							...
							title={t('certificateTitle', {
								title: PreferredAccess.CatalogEntry.title,
							})}
						/>
					</Prompt.Dialog>
				)}
	...
	render() {
		...
		return (
			<HOC.ItemChanges
				item={this.props.course.PreferredAccess}
				onItemChange={this.onPreferredAccessChange}
			>
...
export default decorate(OutlineHeader, [Hooks.afterBatchEvents()]);
