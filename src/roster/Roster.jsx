...
export default class Roster extends React.Component {
		...
		if (batchLinkFor && router && router.routeTo && router.routeTo.path) {
			...
			if (batchLink) {
				router.routeTo.object({
					isProgressBatch: true,
					href: `./progress/${encodeBatchLink(batchLink)}`,
				});
			}
		} else {
			logger.warn(
				'Unable to route to student progress. Ignoring row click.'
			);
		}
	};
	...
	render() {
		...
		const onRowClick = (course || {}).CompletionPolicy
			? this.onRowClick
			: null;
		...
		return (
			<Scroll.BoundaryMonitor
				window
				onBottom={this.onScrolledBottom}
