...
describe('Scorm progress view test', () => {
	const verifyView = async (isInstructor, completed, passed) => {
		...
		if(isInstructor) {
			expect(x.container.querySelector('.circular-progress .number').textContent).toEqual('50');
		}
		else {
			if(completed) {
				...
				if(passed) {
					expect(status.querySelector('.title').textContent).toEqual('Passed');
					expect(status.querySelector('.message').textContent).toEqual('Congratulations, you earned a satisfactory score.');
				}
				else {
					expect(status.querySelector('.title').textContent).toEqual('Bummer..');
					expect(status.querySelector('.message').textContent).toEqual('Unfortunately, you did not earn a satisfactory score.');
				}
			}
			else {
				const status = x.container.querySelector('.not-completed');

				expect(status.querySelector('.title').textContent).toEqual('No Data Yet');
				expect(status.querySelector('.message').textContent).toEqual('Data about your course will appear here.');
			}
		}
